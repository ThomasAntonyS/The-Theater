...
const Search = () => {
  ...
  useEffect(() => {
    ...
    if (q) fetchResults(q, Number(page_no) || 1, searchType);
  }, [q, page_no, searchType]);
  ...
  const fetchResults = async (query, page, type) => {
    if (!query.trim()) return;
    setLoading(true);
    try {
      const response = await fetch(`${backendBaseUrl}/api/search?query=${encodeURIComponent(query)}&page=${page}&type=${type}`);
      ...
      <div className="pt-[15vh] px-6 md:px-12 max-w-[1400px] mx-auto">
        <header className="mb-12">
          <h1 className="text-5xl md:text-8xl font-manrope font-black italic uppercase tracking-tighter text-white leading-none mb-4">
            Search <span className="text-red-600">Library</span>
          </h1>
          <div className="flex gap-4 mt-8">
            {['movie', 'person'].map((t) => (
                <button
                    key={t}
                    onClick={() => { setSearchType(t); setMovies([]); setPageNo(1); }}
                    className={`px-8 py-2 font-manrope font-black uppercase italic tracking-widest text-[10px] md:text-xs rounded-full border-2 transition-all duration-300 ${searchType === t ? 'bg-red-600 border-red-600 text-white' : 'border-white/10 text-white/70 hover:border-white/70'}`}
                >
                    {t === 'movie' ? 'Movies' : 'Cast & Crew'}
                </button>
            ))}
          </div>
        </header>
        ...
        <div className="relative z-50 mb-20">
          <div className="flex w-full sm:max-w-3xl items-end gap-6 border-b-2 border-white/70 focus-within:border-red-600 transition-all duration-500 pb-4">
          ...
          {showSuggestions && suggestions.length > 0 && (
            <div ref={suggestionsRef} className="absolute top-full left-0 w-full bg-[#0a0a0a]/95 backdrop-blur-2xl border border-white/10 rounded-xl mt-4 overflow-hidden shadow-2xl z-[100]">
              {suggestions.map((item) => (
                <div
                  key={item.id}
                  className="px-6 py-4 hover:bg-white/5 cursor-pointer flex items-center border-b border-white/5 transition-colors"
                  onClick={() => {
                    navigate(searchType === 'movie' ? `/movie/${item.id}` : `/cast/${item.id}/${item.known_for_department || 'acting'}`);
                    setShowSuggestions(false);
                  }}
                >
                  <img src={item.poster_path || item.profile_path ? `https://image.tmdb.org/t/p/w92${item.poster_path || item.profile_path}` : 'https://via.placeholder.com/92x138'} className="w-10 h-14 object-cover rounded bg-zinc-900" alt="" />
                  <div className="ml-6">
                    <p className="text-white font-manrope font-bold uppercase text-sm tracking-widest">{item.title || item.name}</p>
                    <p className="text-white/70 font-nunito text-xs mt-1">
                      {searchType === 'movie' ? item.release_date?.slice(0, 4) : item.known_for_department}
                    </p>
                  </div>
                </div>
              ))}
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 md:gap-8">
                {movies.map((item) => (
                  <Link
                    key={item.id}
                    to={searchType === 'movie' ? `/movie/${item.id}` : `/cast/${item.id}/${item.known_for_department || 'acting'}`}
                  ...
                  >
                    ...
                    <div className="aspect-[2/3] overflow-hidden bg-zinc-900">
                      <img src={item.poster_path || item.profile_path ? baseImage + (item.poster_path || item.profile_path) : ''} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt="" />
                    </div>
                    <div className="absolute bottom-0 w-full p-4 bg-gradient-to-t from-black to-transparent">
                      <p className="text-white font-manrope font-black text-sm italic uppercase tracking-tighter truncate">{item.title || item.name}</p>
                      <p className="text-white/50 font-nunito text-[10px] mt-1 font-bold tracking-widest uppercase">
                        {searchType === 'movie' ? item.release_date?.slice(0, 4) : item.known_for_department}
                      </p>
                    </div>
                  </Link>
